...
Store.prototype = {
  ...
  getFromSnapshot: function(streamId, maxRev, callback) {
    ...
    async.waterfall([
      ...
      function getSnapshot(callback) {
        self.storage.getSnapshot(query, maxRev, callback);
      },
      ...
      function getEventStream(snap, callback) {
        ...
        self.getEventStream(query, rev, maxRev, function(err, stream) {
          if (err) callback(err);
  ...
  createSnapshot: function(streamId, revision, data, version, callback) {
    ...
    var snapshot = new Snapshot(null, query, revision, version, data);
  ...
  // __getNewEventStream:__ get an new EventStream.
  // 
  // `eventStore.getNewEventStream(streamId)`
  //
  // - __streamId:__ id for requested stream (equal to aggregateId)
  getNewEventStream: function(streamId) {
    ...
    return new EventStream(this, query, null);
  },
  ...
  getEventStream: function(streamId, revMin, revMax, callback) {
    ...
    var self = this;
    this.storage.getEvents(query, revMin, revMax, function(err, events) {
      if (err) {
        callback(err);
      } else {
        callback(null, new EventStream(self, query, events));
      }
    });
  ...
  // - __callback:__ `function(err, evtsToSetToDispatched){}`
    ...
    async.waterfall([
      ...
      function commitEvents(id, callback) {
        ...
        self.storage.addEvents(eventstream.uncommittedEvents, function(err) {
          ...
          if (self.options.enableDispatching && self.dispatcher) {
  ...
  getEvents: function(streamId, revMin, revMax, callback) {
    ...
    var self = this;
    this.storage.getEvents(query, revMin, revMax, callback);
  ...
  getUndispatchedEvents: function(callback) {
    ...
    this.storage.getUndispatchedEvents(callback);
  },
  ...
  //
  // `eventStore.setEventToDispatched(event, callback)`
  //
  ...
  setEventToDispatched: function(evt, callback) {
    ...
    this.storage.setEventToDispatched(evt, callback);
  }
...
var Event = function(eventStream, event) {
  this.streamId = eventStream.streamId || null;
  this.aggregateId = eventStream.aggregateId || null;
  this.aggregate = eventStream.aggregate || null;
  this.context = eventStream.context || null;
...
var Snapshot = function(id, query, revision, version, data) {
  this.id = id;
  this.streamId = query.aggregateId || null;
  this.aggregateId = query.aggregateId || null;
  this.aggregate = query.aggregate || null;
  this.context = query.context || null;
