...
Store.prototype.use = function(module) {
    if (Util.checkInterface(module, Interfaces.IStorage)) {
        this.storage = module;
    }
    
    if (Util.checkInterface(module, Interfaces.IPublisher)) {
...
Store.prototype.commit = function(eventstream) {
    ...
    for (i = 0, len = events.length; i < len; i++) {
        ...
        // store
        this.storage.addEvent(item);
        
        // publish it now
        if (this.publisher) {
            this.publisher.publish(item.payload);
            this.storage.setEventToDispatched(item);
        }
