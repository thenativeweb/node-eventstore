...
Storage.prototype = {
    ...
    addEvent: function(event, callback) {
        this.events.insert(event, callback);
    },
    ...
    getSnapshot: function(streamId, maxRev, callback) {
        ...
        this.snapshots.find(findStatement, {sort:[['revision','asc']], limit: 1}).toArray(function(err, snapshots) {
            if (err) {
                callback(err);
            } else {
    ...
    setEventToDispatched: function(evt, callback) {
        ...
        this.events.update({'_id' : evt.commitId}, updateCommand, callback);
    },
