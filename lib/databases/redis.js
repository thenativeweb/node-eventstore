...
_.extend(Redis.prototype, {
  ...
  addEvents: function (events, callback) {
    ...
    _.each(events, function (evt) {
      ...
      keysEvtMap.push(self.options.prefix + ':' + self.options.eventsCollectionName + ':' + evt.commitStamp.getTime() + ':' + context + ':' + aggregate + ':' + aggregateId + ':' + evt.commitId);
      keysEvtMap.push(JSON.stringify(evt));
      undispKeysEvtMap.push(self.options.prefix + ':undispatched_' + self.options.eventsCollectionName + ':' + evt.commitStamp.getTime() + ':' + context + ':' + aggregate + ':' + aggregateId + ':' + evt.commitId)
    ...
    var args = keysEvtMap.concat(function(err) {
      if (err) {
        debug(err);
        if (callback) callback(err);
        return;
      }
      var args2 = undispKeysEvtMap.concat(callback);
      
      self.client.mset.apply(self.client, args2);
    });
