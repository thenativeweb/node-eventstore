    ...
    this.events.find(findStatement, { sort: [['commitStamp', 'asc'], ['streamRevision', 'asc'], ['commitSequence', 'asc']] }).toArray(function (err, res) {
      ...
      if (lastEvt.restInCommitStream === 0 || !lastEvt.restInCommitStream) {
        ...
        self.transactions.remove({
          ...
          aggregate: lastEvt.aggregate,
          context: lastEvt.context
        }, function (err) {
          if (err) {
            debug(err);
          }
      ...
      self.getLastEvent(query, function(err, lastFetchedEvt) {
        ...
        if (lastFetchedEvt.restInCommitStream === 0 || !lastFetchedEvt.restInCommitStream) {
          ...
          // the following is usually unnecessary
          self.transactions.remove({
            aggregateId: lastFetchedEvt.aggregateId,
            aggregate: lastFetchedEvt.aggregate,
            context: lastFetchedEvt.context
          }, function (err) {
            if (err) {
              debug(err);
            }
