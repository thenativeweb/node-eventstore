...
_.extend(Mongo.prototype, {
  ...
  addEvents: function (events, callback) {
    ...
    // when inserting new events be sure no other transaction exists
    this.removeTransactions(events[0]);
  },
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    this.events.find(findStatement, { sort: [['commitStamp', 'asc'], ['streamRevision', 'asc'], ['commitSequence', 'asc']] }).toArray(function (err, res) {
      ...
      if (lastEvt.restInCommitStream === 0 || !lastEvt.restInCommitStream) {
        ...
        return self.removeTransactions(lastEvt);
      }
      ...
      if (revMax === -1) {
        debug('revMax is -1 => repair');
        return repair();
      }

      if (lastEvt.streamRevision === revMax) {
        debug('streamRevision of last event is equal revMax => everything is fine');
        return callback(null, res);
      }

      debug('revMax is set => getLastEvent');
      self.getLastEvent(query, function(err, lastFetchedEvt) {
        if (err) {
          debug(err);
          return callback(err);
        }

        if (lastFetchedEvt.restInCommitStream === 0 || !lastFetchedEvt.restInCommitStream) {
          callback(err, res);

          // the following is usually unnecessary
          return self.removeTransactions(lastFetchedEvt);
        }

        debug('new fetched last event is not last => repair');
        repair();
      });
  ...
  getPendingTransactions: function (callback) {
    this.transactions.find({}).toArray(callback);
  },
