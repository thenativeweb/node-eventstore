...
_.extend(DynamoDB.prototype, {
  ...
  clear: function (done) {
    ...
    var clearEvents = function(callback) { 
      clearEventTables(self.options, self.client, function(err) {
        if(err) {
          debug(err);
          return callback(err);
        }
    ...
    var clearSnapshots = function(callback) { 
      clearSnapshotsTable(self.options, self.client, function(err) {
        if(err) {
          debug(err);
          return callback(err);
        }
  ...
  addEvents: function (events, callback) {
    ...
    //TODO: use batchWrite w/ retry for UnprocessedItems
    async.concat(events,
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    async.doWhilst(function (end) {
      ...
      if (tableQuery.KeyConditionExpression) {
        ...
        //TODO:  optimize access patterns. see: https://medium.com/building-timehop/one-year-of-dynamodb-at-timehop-f761d9fe5fa1
        // possibly use a global secondary index?
        client.scan(tableQuery, function (err, results) {
          ...
          //TODO: (scan only) default limit is 100 - use exponential backoff if result.Count is < 100 and result.LastEvaluatedKey != 100
  ...
  getEventsSince: function (date, skip, limit, callback) {
    ...
    async.doWhilst(function (end) {
      ...
      client.scan(tableQuery, function (err, results) {
        ...
        //TODO: default limit is 100 - use exponential backoff if result.Count is < 100 and result.LastEvaluatedKey != 100
...
var clearEventTables = function (opts, client, cleared) {
  var exclusiveStartKey = null;
  var retryCount = 0;

  //TODO: read from undispatchedEvent tables first - batch deleting both - then read/delete from only events (reduce consumed capacity)
  var read = function(query, callback) {
    client.scan(query, function (err, page) {
      ...
      if(page.Count < query.Limit && exclusiveStartKey) {
        debug("read capacity limit reached: " + JSON.stringify(page, null, 2));
      }
      ...
      var ids = _.map(page.Items, function(n) {
      ...
      batch.RequestItems[opts.eventsTableName] = keys;
      batch.RequestItems[opts.undispatchedEventsTableName] = ids;
    ...
    if(batch && batch.RequestItems){
      client.batchWriteItem(batch, function(err, result) {
        ...
        if(Object.keys(result.UnprocessedItems).length !== 0) {
          debug("Retry batchWriteItem: " + JSON.stringify(result, null, 2));
          retryCount++;
          var retry = {
  ...
  var params = {
    ...
    Limit: 12, // max 25 per batchWriteItem call divided by 2 tables
...
var clearSnapshotsTable = function (opts, client, cleared) {
  var exclusiveStartKey = null;

  var query = {
    TableName: opts.snapshotsTableName,
    ProjectionExpression: "aggregateId,id",
    Limit: 25, // max 25 per batchWriteItem call
    ConsistentRead: false,
    ReturnConsumedCapacity: "TOTAL"
  };
  ...
  async.doWhilst(function (end) {
    ...
    client.scan(query, function (err, page) {
      ...
      //TODO: use 'documentClient' instead
      client.batchWriteItem(batch, function (error, data) {
        return end(error);
      });
    });
  }, function () {
