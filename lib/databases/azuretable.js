...
function AzureTable(options) {
  ...
  if (process.env.TRAVIS_AZURE_TABLE_PREFIX) {
    this.options.eventsTableName += process.env.TRAVIS_AZURE_TABLE_PREFIX + process.pid;
    this.options.snapshotsTableName += process.env.TRAVIS_AZURE_TABLE_PREFIX + process.pid;
  }
}
...
//        if (done) done(null); // strange on azure...
  ...
  addEvents: function (events, callback) {
    ...
    this.client.executeBatch(this.options.eventsTableName, batch, function (err, performBatchOperationResponses, batchResponse) {
      if (err) {
        debug(err);
        if (callback) callback(err);
        return;
