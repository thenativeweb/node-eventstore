...
_.extend(Tingo.prototype, {
  ...
  addEvents: function (events, callback) {
    ...
    _.each(events, function (evt) {
      ...
      evt._id = evt.commitId + (evt.commitSequence || 0);
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    if (limit === -1) {
      return this.events.find(findStatement, { sort:[[ 'commitStamp','asc' ], [ 'streamRevision','asc' ]] }).skip(skip).toArray(callback);
    }

    this.events.find(findStatement, { sort:[[ 'commitStamp','asc' ], [ 'streamRevision','asc' ]] }).skip(skip).limit(limit).toArray(callback);
  },
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    this.events.find(findStatement, { sort:[[ 'commitStamp','asc' ], [ 'streamRevision','asc' ]] }).toArray(function (err, res) {
      if (err) {
        debug(err);
        return callback(err);
      }
      ...
      self.transactions.findOne({ _id: lastEvt.commitId }, function (err, tx) {
        ...
        self.events.insert(missingEvts, {keepGoing: true}, function (err) {
          ...
          self.getEvents(query, revMin, maxRev, callback);
        });
      });
    });
  },
  ...
  getUndispatchedEvents: function (callback) {
    this.events.find({ 'dispatched' : false }, { sort:[[ 'commitStamp','asc' ], [ 'streamRevision','asc' ]] }).toArray(callback);
  },

  setEventToDispatched: function (id, callback) {
    var updateCommand = { '$set' : { 'dispatched': true } };
    this.events.update({'commitId' : id}, updateCommand, callback);
  },
  ...
  getSnapshot: function (query, revMax, callback) {
    ...
    this.snapshots.findOne(findStatement, { sort:[['revision','asc']] }, callback);
  }
