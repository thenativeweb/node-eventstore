 ...
 */
   ...
   * @param {string} [parentEventId] - optional parentEventId to find related parent event
   */
  function enhanceCallback(name, callback, args, eventId, parentEventId) {
    if (callback) {
      var originalCallback = callback;

      callback = function () {
        self.eventstore.emit.call(self.eventstore, 'after-' + name, getEmitArguments(args, eventId, parentEventId));
  ...
  function addEventEmitterToMethod(name, originalMethod, args, callback) {
    ...
    callback = enhanceCallback(name, callback, args, self.eventId, parentEventId);
