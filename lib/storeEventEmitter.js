...
function addStoreEventEmitter(eventstore) {
   ...
   */
  function addEventEmitter(name, originalMethod, args, callback, addEventId) {
    var self = this;
    ...
    if (callback) {
      callback = function () {
        eventstore.emit.apply(eventstore, _.concat('after-' + name, Date.now(), args, self.eventId || []));
        return originalCallback.apply(this, arguments);
      };
    }

    eventstore.emit.apply(eventstore, _.concat('before-' + name, Date.now(), args, self.eventId || []));
  ...
  if (eventstore.streamEventsByRevision) {
    ...
    eventstore.streamEventsByRevision = function (date, revMin, revMax) {
