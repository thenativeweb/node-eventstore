...
Store.prototype = {
    ...
    // __getEventStreams:__ loads the eventstreams.
    // 
    // `eventStore.getEventStreams(type, minRev, maxRev, callback)`
    //
    // - __type:__ type for requested streams (equal to saga type)
    // - __callback:__ `function(err, eventStream){}`
    getEventStreams: function(type, callback) {
        
        if (this.hasConfigurationErrors(callback)) return;
        
        var self = this;
        this.storage.getEventsOfType(type, function(err, events) {
            if (err) {
                callback(err);
            } else {
                var streams = {};
                for (var i = 0, len = events.length; i < len; i++) {
                    var evt = events[i];
                    if(!streams[evt.streamId]) {
                        streams[evt.streamId] = [];
                    }
                    streams[evt.streamId].push(evt);
                }
                var eventStreams = [];
                for (var streamId in streams) {
                    eventStreams.push(new EventStream(self, streamId, streams[streamId]));
                }
                callback(null, eventStreams);
            }
        });
    ...
    commit: function(eventstream, callback) {
        ...
        async.waterfall([
            ...
            function commitEvents(id, callback) {
                ...
                self.storage.addEvents(eventstream.uncommittedEvents, eventstream.type, function(err) {
                    if (err) callback(err); 
                });
    ...
    // __remove:__ removes all events in the eventstream.
    //
    // __hint:__ directly use the remove function on eventstream
    // 
    // `eventStore.remove(eventstream, callback)`
    //
    // - __eventstream:__ the eventstream object
    // - __callback:__ `function(err, eventStream){}`
    remove: function(eventstream, callback) {
    
        if (this.hasConfigurationErrors(callback)) return;
    
        var self = this;

        this.storage.removeEvents(eventstream.streamId, callback);
...
EventStream.prototype = {
    ...
    // __remove:__ removes all events of this stream
    //
    // `eventstream.remove(callback)
    //
    // - __callback:__ `function(err){}` [optional]
    remove: function(callback) {
        this.store.remove(this, callback);
    }
};
