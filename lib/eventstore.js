...
_.extend(Eventstore.prototype, {
  ...
  commit: function(eventstream, callback) {
    ...
    waterfallFuncs.push(function getNewId(callback) {
        self.getNewId(callback);
    });

    // Generate an id for each uncommitted event
    for (var i= 0, len = eventstream.uncommittedEvents.length; i < len; i++) {
      waterfallFuncs.push(function getAnotherId(previousId, callback) {
        idSet.push(previousId);
        self.getNewId(callback);
      });
    }
    waterfallFuncs.push(function commitEvents(id, callback) {
      idSet.push(id);
      ...
      self.store.getNextPositions(uncommittedEvents.length, function(err, positions) {
        ...
        for (var i = 0, len = uncommittedEvents.length; i < len; i++) {
          ...
          event.id = idSet[i+1]; // additional ids generated by getAnotherId function
          event.commitId = idSet[0]; // first id generated is for the commit
