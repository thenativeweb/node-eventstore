  ...
  createSnapshot: function(obj, callback) {
    ...
    if (obj.revision) {
      if (typeof (obj.revision) !== 'number') {
        ...
        if (castedRevision && castedRevision == obj.revision) { // Determines if the revision was parsed correctly, for the cases where user using custom typed revisions that's not in valid float format.
